...
class A(SingleFactor):
    def generate_factor(self):
        a = DataFrame({stock: pd.read_csv('../../DataBase/StockTradingDerivativeData/Stock/%s.csv'%stock, index_col=[0], parse_dates=[0]).loc[:, 'PSTTM'] for stock in self.stocks})
        a = 1 / a
...
a = A('SP', stocks=stocks, start_date='20200101', end_date='20201010')
