...
class SingleFactor:
    ...
    def factor_analysis(self, industry_neutral=True, size_neutral=True, num_group=5):
        ...
        #市值中性
        if size_neutral:
            market_capitalization = DataFrame({stock: pd.read_csv('../../DataBase/StockTradingDerivativeData/Stock/%s.csv'%stock, index_col=[0], parse_dates=[0]).loc[:, 'TOTMKTCAP'] for stock in self.stocks})
        ...
        plt.savefig('../Results/%s/hist.png'%self.factor_name)
