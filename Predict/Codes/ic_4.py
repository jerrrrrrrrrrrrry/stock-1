...
if __name__ == '__main__':
    ...
    ic_mega = pd.read_csv('%s/Results/ic_mega.csv'%gc.IC_PATH, index_col=[0], parse_dates=[0])
    ic_big = pd.read_csv('%s/Results/ic_big.csv'%gc.IC_PATH, index_col=[0], parse_dates=[0])
    ic_small = pd.read_csv('%s/Results/ic_small.csv'%gc.IC_PATH, index_col=[0], parse_dates=[0])
    ic_micro = pd.read_csv('%s/Results/ic_micro.csv'%gc.IC_PATH, index_col=[0], parse_dates=[0])
    ...
    n = 5
    lag = n + 1
    ic_mega_mean_hat = ic_mega.ewm(halflife=halflife_mean).mean().shift(lag)
    ic_big_mean_hat = ic_big.ewm(halflife=halflife_mean).mean().shift(lag)
    ic_small_mean_hat = ic_small.ewm(halflife=halflife_mean).mean().shift(lag)
    ic_micro_mean_hat = ic_micro.ewm(halflife=halflife_mean).mean().shift(lag)
    for factor in factors:
        ...
        r_hat = r_hat.add(factor_df.mul(ic_micro_mean_hat.loc[:, factor], 0), fill_value=0)
        r_hat = r_hat.add(small_mask * factor_df.sub(knot_1, 0).mul(ic_small_mean_hat.loc[:, factor] - ic_micro_mean_hat.loc[:, factor], 0), fill_value=0)
        r_hat = r_hat.add(big_mask * factor_df.sub(knot_2, 0).mul(ic_big_mean_hat.loc[:, factor] - ic_small_mean_hat.loc[:, factor], 0), fill_value=0)
        r_hat = r_hat.add(mega_mask * factor_df.sub(knot_3, 0).mul(ic_mega_mean_hat.loc[:, factor] - ic_big_mean_hat.loc[:, factor], 0), fill_value=0)
        
    r_hat.to_csv('../Results/r_hat_ic_4.csv')