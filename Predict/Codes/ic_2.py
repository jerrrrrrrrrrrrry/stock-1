...
if __name__ == '__main__':
    ...
    ic_pos = pd.read_csv('%s/Results/ic_pos.csv'%gc.IC_PATH, index_col=[0], parse_dates=[0])
    ic_neg = pd.read_csv('%s/Results/ic_neg.csv'%gc.IC_PATH, index_col=[0], parse_dates=[0])
    ...
    n = 5
    lag = n + 1
    ic_pos_mean_hat = ic_pos.ewm(halflife=halflife_mean).mean().shift(lag)
    ic_neg_mean_hat = ic_neg.ewm(halflife=halflife_mean).mean().shift(lag)
    for factor in factors:
        factor_df = factor_df_dic[factor]
        knot_1 = factor_df.quantile(0.5, 1)
        pos_mask = factor_df.ge(knot_1, 0)
        
        r_hat = r_hat.add(factor_df.mul(ic_neg_mean_hat.loc[:, factor], 0), fill_value=0)
        r_hat = r_hat.add(pos_mask * factor_df.sub(knot_1, 0).mul(ic_pos_mean_hat.loc[:, factor] - ic_neg_mean_hat.loc[:, factor], 0), fill_value=0)
        
    r_hat.to_csv('../Results/r_hat_ic_2.csv')