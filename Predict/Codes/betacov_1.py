...
if __name__ == '__main__':
    ...
    for date in weight.index:
        weight.loc[date, :] = np.linalg.inv(beta_cov_hat.loc[date, :, :].values + lamb*np.eye(len(factors))).dot(beta_mean_hat.loc[date, :].values)
            
    for factor in factors:
        r_hat = r_hat.add(factor_df_dic[factor].mul(weight.loc[:, factor], 0), fill_value=0)
    
    r_hat.to_csv('../Results/r_hat_betacov_1.csv')
