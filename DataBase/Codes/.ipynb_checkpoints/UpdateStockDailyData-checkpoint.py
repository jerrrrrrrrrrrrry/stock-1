...
i = 0
while 99 * i < len(stocks):
    adj_factor = pd.concat([adj_factor, pro.adj_factor(ts_code=', '.join(list(stocks)[99*i:99*i+99]), start_date=start, end_date=end, fields='ts_code, adj_factor').set_index('ts_code')], axis=0, sort=False)
    i += 1
...
for stock in stocks:
    if (stock in list(daily.index)) and (stock in list(adj_factor.index)) and (stock in list(daily_basic.index)):
        df = pd.concat([daily.loc[[stock], :], adj_factor.loc[[stock], :], daily_basic.loc[[stock], :], ], axis=1, sort=False)
