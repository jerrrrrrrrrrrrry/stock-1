...
def init(context):
    ...
    for ind in stocks.index:
        data = get_fundamentals(table='prim_finance_indicator', symbols=symbols.loc[ind], start_date=start_date, end_date=end_date, 
                fields='EBIT, EBITDA, EBITSCOVER, EBITDASCOVER, ROEDILUTED, ROEDILUTEDCUT, ROEWEIGHTED, ROEWEIGHTEDCUT', df=True)
        if len(data) > 0:
            ...
            data.to_csv('D:/stock/DataBase/StockPrimFinanceData/Stock/%s.csv'%(filename.loc[ind]))
            daily_df = pd.concat([daily_df, data.iloc[-1:]], axis=0, sort=False)
        print(stocks.loc[ind], 'Downloaded')
    daily_df.to_csv('D:/stock/DataBase/StockPrimFinanceData/Daily/%s.csv'%(daily_df.index[-1]))

if __name__ == '__main__':
    run(strategy_id='64018e9a-0554-11eb-81e8-1a259301b6c6',
        filename='prim_finance.py',
        mode=MODE_BACKTEST,
